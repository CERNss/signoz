...
func (a *AuthN) LoginURL(ctx context.Context, siteURL *url.URL, authDomain *authtypes.AuthDomain) (string, error) {
	...
	state, err := signStateValue(authtypes.NewState(siteURL, authDomain.StorableAuthDomain().ID).URL.String(), a.stateSigningSecret(authDomain))
	if err != nil {
		return "", err
	}
...
func (a *AuthN) HandleCallback(ctx context.Context, query url.Values) (*authtypes.CallbackIdentity, error) {
	...
	if err := verifySignedStateSignature(payload, signature, a.stateSigningSecret(authDomain)); err != nil {
		return nil, errors.Newf(errors.TypeInvalidInput, authtypes.ErrCodeInvalidState, "oidc: invalid state").WithAdditional(err.Error())
	}
	...
	emailClaimName := authDomain.AuthDomainConfig().OIDC.ClaimMapping.Email
	if emailClaimName == "" {
	...
	if authDomain.AuthDomainConfig().OIDC.GetUserInfo || !hasEmailInIDToken {
	...
	switch authDomain.AuthDomainConfig().OIDC.EffectiveEmailVerifiedPolicy() {
	...
	if authDomain.AuthDomainConfig().OIDC.EnforceEmailDomain && !emailDomainMatches(email.StringValue(), authDomain.StorableAuthDomain().Name) {
	...
	name := resolveName(claims, authDomain.AuthDomainConfig().OIDC.ClaimMapping.Name)

	groups := extractGroups(ctx, claims, authDomain.AuthDomainConfig().OIDC.ClaimMapping.Groups, a.settings.Logger())
...
func (a *AuthN) oidcProviderAndOAuth2Config(ctx context.Context, siteURL *url.URL, authDomain *authtypes.AuthDomain) (*oidc.Provider, *oauth2.Config, error) {
	...
	scopes := authDomain.AuthDomainConfig().OIDC.EffectiveScopes()
...
func (a *AuthN) stateSigningSecret(authDomain *authtypes.AuthDomain) string {
	return authDomain.AuthDomainConfig().OIDC.ClientSecret
}
