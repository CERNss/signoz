		...
		for _, authDomain := range authDomains {
			...
			if err != nil {
				module.settings.Logger().WarnContext(
					...
					errors.Attr(err),
					slog.Any("authn_provider", authDomain.Kind()),
...
func (module *module) CreateCallbackAuthNSession(ctx context.Context, authNProvider authtypes.AuthNProvider, values url.Values) (string, error) {
	...
	if authDomain.Kind() == authtypes.AuthNProviderOIDC {
		...
		allowJITProvisioning = oidcConfig.IsAllowJIT()
	}
...
func (module *module) GetSessionLogoutContext(ctx context.Context, siteURL *url.URL) (*authtypes.SessionLogoutContext, error) {
	...
	logoutProvider, ok := module.authNs[authDomain.Kind()].(authn.LogoutURLProvider)
	if !ok {
		return authtypes.NewSessionLogoutContext(""), nil
	}
	...
	if err != nil {
		module.settings.Logger().WarnContext(ctx, "failed to compute provider logout URL", errors.Attr(err), slog.Any("authn_provider", authDomain.Kind()))
		return authtypes.NewSessionLogoutContext(""), nil
	}
