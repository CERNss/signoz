...
if [ "${1:-}" = "--full" ]; then
  ...
  GOTOOLCHAIN="$GO_MOD_TOOLCHAIN" go test ./pkg/authn/callbackauthn/oidccallbackauthn/... ./pkg/types/authtypes/... ./pkg/modules/session/implsession/... \
  ...
  if [ -d frontend/node_modules ]; then
    (cd frontend && pnpm exec jest src/container/Login --silent) \
      || { FAIL=$((FAIL + 1)); FAILED_ITEMS+=("[测试失败] frontend Login tests"); }
  else
